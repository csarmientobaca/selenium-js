...
function assertLinkNotValid(link) {
    const httpModule = link.startsWith('https') ? require('https') : require('http'); // Require the 'https' or 'http' module
    ...
    return new Promise((resolve, reject) => {
        httpModule.get(link, (response) => {
            ...
            if (response.statusCode === 200) {
                ...
                try {
                    const responseUrl = response.responseUrl || link; // Use link if responseUrl is undefined
                    console.log(link);
                    ...
                    if (response.headers['x-cache'] === 'Error from cloudfront') {
                        ...
                        console.log('Link is valid.insideeee');
                        resolve();
                    } else {
                ...
                console.log('Link is invalid.');  // Print when the link returns a non-200 status code
...
// Example usage
const inputUrl = 'https://miniclip.com/games/web';

assertLinkNotValid(inputUrl)
    .then(() => {
        console.log('Link is valid.out');
    })
    .catch((error) => {
